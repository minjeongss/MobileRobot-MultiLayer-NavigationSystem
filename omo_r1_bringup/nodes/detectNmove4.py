...
class cmd_pose:
    ...
    def callback(self, tag):
        if tag.detections:
            try:
                self.listener.waitForTransform('/base_link', '/num2', rospy.Time(0), rospy.Duration(5.0))
                (trans, rot) = self.listener.lookupTransform('/base_link', '/num2', rospy.Time(0))
                x = trans[0]
                y = trans[1]
                z = trans[2]

                distance = math.sqrt(x ** 2 + y ** 2)
                target_distance = 1.15# Target distance in meters
