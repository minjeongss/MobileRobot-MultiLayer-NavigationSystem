...
class PacketWriteHandler:
   ...
   def write_register(self, param1, param2):
      self.write_packet("$cREGI," + str(param1) + ',' + param2)
      sleep(0.05)

   def write_periodic_query_value(self, param):
      self.write_packet("$cPERI," + str(param))
      sleep(0.05)

   def write_periodic_query_enable(self, param):
      self.write_packet("$cPEEN," + str(param))
      sleep(0.05)

   def write_init_odometry(self):
      self.write_packet("$cODO")
      sleep(0.05)
   ...
   def stop_peen(self):
      self.write_packet("$cPEEN, 0")
      sleep(0.05)

   def stop_callback(self):
      self.write_packet("$cCBEN, 0")
      sleep(0.05)
...
class PacketReadHandler:
   ...
   def read_packet(self):
      if self._ph.get_port_state() == True:
         whole_packet = self._ph.read_port().decode("utf-8").strip('\r\n')
         if whole_packet:
...
class PortHandler():
   ...
   def set_port_handler(self, port_name, baud_rate):
      self._ser = serial.Serial(port_name, baud_rate,timeout=0.01)
